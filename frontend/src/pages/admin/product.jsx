...
export default function Product() {
  ...
  function handleDelete(key) {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const token = localStorage.getItem("token");
      axios
        .delete(`http://localhost:4000/api/product/${key}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          setItemLoaded(false);
  ...
  return (
    <div className="w-full h-screen flex flex-col ">
      ...
      <div className="w-full h-16 flex flex-row justify-end items-center ">
        ...
        <Link to="/admin/item/addItem">
          <button>
            <AiOutlineAppstoreAdd className="text-[40px] hover:text-[#a24af8] hover:animate-bounce mr-12 hover:" />
          </button>
        </Link>
      </div>
      <table className="border-collapse border border-gray-300 shadow-lg rounded-lg w-full text-left overflow-hidden">
        ...
        <tbody>
          {items.map((product) => (
            <tr
              key={product.key}
              className="odd:bg-white even:bg-gray-100 hover:bg-purple-100"
            >
              ...
              <td className="p-3 border border-gray-300">
                {product.warenty_Period}
              </td>
              <td className="p-3 border border-gray-300">
                {product.isApprove ? "Approved" : "notApproved"}
              </td>
              <td className="p-3 border border-gray-300">
                <img
                  src={product.Image}
              ...
              <td className="p-3 border border-gray-300">
                <button
                  className=" hover:text-white bg-blue-500 h-14 w-16 rounded-xl mb-3"
                  onClick={() => {
                ...
                <button
                  className=" hover:text-white bg-red-500 h-14 w-16 rounded-xl"
                  onClick={() => {
